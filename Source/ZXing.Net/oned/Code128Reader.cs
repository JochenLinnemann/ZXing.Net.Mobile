    ...
    public sealed class Code128Reader : OneDReader
    {
        ...
        override public Result decodeRow(int rowNumber, BitArray row, IDictionary<DecodeHintType, object> hints)
        {
            ...
            if (!row.isRange(nextStart,
                             ...
                             // JL, 2015-10-05: only check for 25% of last pattern size
                             Math.Min(row.Size, nextStart + (nextStart - lastStart) / 4),
                             false))
            {
                Debug.WriteLine("not enough trailing whitespace");
                return null;
            }
                ...
                else
                {
                    if (counterPosition == patternLength - 1)
                    {
                        ...
                        if (bestMatch >= 0)
                        {
                            ...
                            // JL, 2015-10-05: only check for 25% of first pattern size
                            //if (row.isRange(Math.Max(0, patternStart - (i - patternStart) / 2), patternStart,
                            if (row.isRange(Math.Max(0, patternStart - (i - patternStart) / 4), patternStart,
                                false))
                            {
                            ...
                            {
                                Debug.WriteLine("not enough leading whitespace in current row section");
                            }
                        }
